...
## التشغيل المستمر كل نصف ساعة

اضبط `POST_INTERVAL_MINUTES=30` ثم استخدم أمرًا واحدًا:

```bash
python bot.py
```

ينشر الوكيل فورًا عند بدء التشغيل ثم كل 30 دقيقة، ويدوّر بين `CONTENT_TOPICS`. إذا لم يجد مصدرًا جديدًا، ينشئ منشورًا تعليميًا أصليًا من المحور الحالي. إذا فشل اتصال أو نشر في دورة، يسجل الخطأ ويواصل الدورات التالية.
